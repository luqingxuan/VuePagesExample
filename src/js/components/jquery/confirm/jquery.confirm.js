...
(function($) {
	...
	Jconfirm = function(options) {
		/*
		 * constructor function Jconfirm, options = user options.
		 */
		$.extend(this, options);
		this._init();
	};
	Jconfirm.prototype = {
		...
		setContent : function(string) {
			...
			if (typeof this.content === 'boolean') {
				...
				console.error('Invalid option for property content, passed: '
						+ typeof this.content);
			}
		...
		reactOnKey : function key(e) {
			...
			if (this.contentDiv.find(':input').is(':focus')
					&& /13|32/.test(key))
				return false;
		...
		close : function() {
			...
			var closeTimer = (this.closeAnimation == 'anim-none') ? 0
					: this.animationSpeed;
			setTimeout(function() {
				that.$el.remove();
			}, closeTimer + 50); // wait 50 miliseconds more, for the
									// animation to end gracefully.
